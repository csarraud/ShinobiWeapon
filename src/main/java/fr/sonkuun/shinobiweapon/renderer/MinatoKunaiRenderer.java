...
public class MinatoKunaiRenderer extends EntityRenderer<MinatoKunaiEntity> {

	public static final ResourceLocation MINATO_KUNAI_MODEL_RESOURCE_LOCATION = new ResourceLocation(ShinobiWeapon.MODID, "entity/minato_kunai");
	...
	public MinatoKunaiRenderer(EntityRendererManager renderManager) {
		super(renderManager);
	}

	@Override
	public ResourceLocation getEntityTexture(MinatoKunaiEntity entity) {
		return MINATO_KUNAI_MODEL_RESOURCE_LOCATION;
	}
	
	public void render(MinatoKunaiEntity kunaiEntity, float entityYaw, float partialTicks,
			MatrixStack matrixStack, IRenderTypeBuffer renderBuffers, int packedLightIn) {
		
		IBakedModel kunaiModel = Minecraft.getInstance().getModelManager().getModel(MINATO_KUNAI_MODEL_RESOURCE_LOCATION);
		matrixStack.push();
